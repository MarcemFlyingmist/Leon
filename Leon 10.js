...
document.querySelector('#hotestChick').addEventListenter('click', hide)
...
function Player_1_Turn(){
    ...
    document.querySelector('#placeToPutResult').innerText = Player_1_Space
    // cout find"place" in doc make Text with innertext set = total
}

function check() {
    const day = document.querySelector('#day').value.toLowerCase() 
    //Worked when hardcoded
    if(day === "tuesday" || day === "thursday"){
        document.querySelector('#placeToPutDay').innerText = "Class Day"
    }
    else if(day === "saturday" || day === "sunday"){
        document.querySelector('#placeToPutDay').innerText = "Weekend"
    }
    else{document.querySelector('#placeToPutDay').innerText = "Boring"}
